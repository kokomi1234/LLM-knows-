...
def get_probe_pred_per_resample(model, tokenizer, clf, layer, token, model_output_greedy, all_textual_answers,
    ...
    for textual_answers, input_output_ids, exact_answer, validity_of_exact_answer in zip(all_textual_answers,
        ...
        # 替换调用 compute_correctness_fn的正确逻辑，匹配不同的数据集
        if compute_correctness_fn.__name__ == 'compute_correctness_winobias':
            # winobias 需要特殊处理，传递三个参数
            if 'incorrect_answer' in model_output_greedy:
                wrong_labels = model_output_greedy.incorrect_answer.to_numpy()
            else:
                # 如果没有 incorrect_answer 列，创建空标签
                wrong_labels = [""] * len(textual_answers[:limit_samples])
            
            # 正确传递三个参数
            result = compute_correctness_fn(textual_answers[:limit_samples], correct_answers.to_numpy(), wrong_labels)
            correctness = result['correctness']
        elif compute_correctness_fn.__name__ == 'compute_correctness_winogrande':
            # winogrande 也需要传递 model_name
            if 'incorrect_answer' in model_output_greedy:
                wrong_labels = model_output_greedy.incorrect_answer.to_numpy()
            else:
                wrong_labels = [""] * len(textual_answers[:limit_samples])
            
            result = compute_correctness_fn(textual_answers[:limit_samples], correct_answers.to_numpy(), wrong_labels, model_name)
            correctness = result['correctness']
        elif 'incorrect_answer' in model_output_greedy:
            # 其他数据集，有 incorrect_answer 的情况
            correctness = compute_correctness_fn(textual_answers[:limit_samples], correct_answers.to_numpy())['correctness']
        else:
            # 其他数据集，无 incorrect_answer 的情况
            correctness = compute_correctness_fn(textual_answers[:limit_samples], correct_answers.to_numpy())['correctness']
        # if 'incorrect_answer' in model_output_greedy:
...
def main():
    ...
    # 数据结构转换 - 将error_stats从字典转为字典列表
    df_error_stats = pd.DataFrame.from_dict(error_stats)
    error_stats_list = df_error_stats.to_dict('records')

    # 使用转换后的数据结构调用函数
    correctness_with_majority = compute_correctness_majority(n_questions, error_stats_list)
    ...
    # 继续使用之前创建的DataFrame直接添加列
    df_error_stats["chosen_answer_probe"] = chosen_answer
    df_error_stats["probe_correctness"] = correctness_with_probing
    df_error_stats["random_choosing_correctness"] = correctness_with_random_choosing
    df_error_stats["majority_correctness"] = correctness_with_majority
    df_error_stats["chosen_answer_full_probe"] = chosen_answer_full

    # 保存结果
    df_error_stats.to_csv(
        f"../output/resampling/{MODEL_FRIENDLY_NAMES[args.model]}_{args.dataset}-{args.n_resamples}_clusters.csv")
