...
from probing_utils import load_model_and_validate_gpu, LIST_OF_DATASETS, LIST_OF_TEST_DATASETS,compile_probing_indices, \
...
def fit_predict(NUANCED_TYPES_OF_MISTAKES, NUANCED_TYPES_OF_MISTAKES_TEST, TYPES_OF_MISTAKES,
    ...
    for type_of_mistake in TYPES_OF_MISTAKES:
        ...
        # 检查样本数量
        sample_count = np.sum(TYPES_OF_MISTAKES[type_of_mistake])
        if sample_count == 0:
            print(f"跳过 {type_of_mistake} - 没有样本")
            # 为结果字典填充默认值
            metrics = {'accuracy': 0, 'precision': 0, 'recall': 0, 'f1': 0, 'auc': 0.5}
            metrics_test = {'accuracy': 0, 'precision': 0, 'recall': 0, 'f1': 0, 'auc': 0.5}
            for m in metrics:
        ...
        if len(unique_classes) < 2:
            print(f"跳过 {type_of_mistake} - 只有一个类别 ({unique_classes[0]})")
            # 为结果字典填充默认值
            metrics = {'accuracy': float(unique_classes[0]), 'precision': 0, 'recall': 0, 'f1': 0, 'auc': 0.5}
            metrics_test = {'accuracy': float(unique_classes[0]), 'precision': 0, 'recall': 0, 'f1': 0, 'auc': 0.5}
            for m in metrics:
                results[f'{type_of_mistake}_{m}'] = metrics[m]
                results[f'{type_of_mistake}_{m}_test'] = metrics_test[m]
            continue
    ...
    if args.merge_types:
        for type_of_mistake in NUANCED_TYPES_OF_MISTAKES:
            ...
            # 检查该类型是否有样本
            if np.sum(TYPES_OF_MISTAKES[type_of_mistake]) == 0:
                print(f"跳过 BETWEEN {type_of_mistake} - 没有样本")
                default_metrics = {'accuracy': 0, 'precision': 0, 'recall': 0, 'f1': 0, 'auc': 0.5}
                for m in default_metrics:
                    results[f'between_types_{type_of_mistake}_{m}'] = default_metrics[m]
                    results[f'between_types_{type_of_mistake}_{m}_test'] = default_metrics[m]
                continue
            ...
            if len(unique_classes) < 2:
                print(f"跳过 BETWEEN {type_of_mistake} - 只有一个类别: {unique_classes[0]}")
                dominant_class = unique_classes[0]
                accuracy = np.mean(y_test_ == dominant_class)
                default_metrics = {
                    'accuracy': accuracy, 
                    'precision': float(dominant_class), 
                    'recall': float(dominant_class),
                    'f1': float(dominant_class), 
                    'auc': 0.5
                }
                for m in default_metrics:
                    results[f'between_types_{type_of_mistake}_{m}'] = default_metrics[m]
...
def main():
    ...
    textual_answers_test = torch.load(f"../output/resampling/{MODEL_FRIENDLY_NAMES[args.model]}_{args.dataset}_{args.n_resamples}_textual_answers.pt")
    exact_answers_test = torch.load(f"../output/resampling/{MODEL_FRIENDLY_NAMES[args.model]}_{args.dataset}_{args.n_resamples}_exact_answers.pt")
    model_output_greedy_test = pd.read_csv(f'../output/{MODEL_FRIENDLY_NAMES[args.model]}-answers-{args.dataset}.csv')
    input_output_ids_greedy_test = torch.load(
            f"../output/{MODEL_FRIENDLY_NAMES[args.model]}-input_output_ids-{args.dataset}.pt")

    error_stats = get_error_stats(textual_answers, exact_answers, model_output_greedy, CORRECTNESS_FN[args.dataset.replace('_test', '')])
    ...
    error_stats_test = get_error_stats(textual_answers_test, exact_answers_test, model_output_greedy_test, CORRECTNESS_FN[args.dataset.replace('_test', '')])
    error_stats_test = pd.DataFrame.from_dict(error_stats_test)
    error_stats_test.to_csv(f"{MODEL_FRIENDLY_NAMES[args.model]}_{args.dataset}_clusters.csv")
