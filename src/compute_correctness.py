...
def compute_correctness_imdb(model_answers, labels):
    # 将映射调整为与数据集一致: positive->1, negative->0
    ...
    for ans in model_answers:
        ...
        try:
            ...
            # 以字符串形式存储数值标签
            exact_answers.append(str(name_to_labels[exact_answer]))
        except KeyError:
            ...
            if negative_index == -1 and positive_index == -1:
                ...
                pred.append(0)  # 'negative' 对应 0
                exact_answers.append('0')
            else:
                pred.append(1)  # 'positive' 对应 1
        ...
        try:
            if type(labels[idx]) == str:
                ...
                try:
                    ...
                    import ast
                    labels_ = ast.literal_eval(labels[idx])
                except SyntaxError:
                    # 处理语法错误
                    print(f"处理标签时出现语法错误: {labels[idx][:30]}...")
                    # 使用字符串清理和保守回退策略
                    if '[' in labels[idx] and ']' in labels[idx]:
                        # 尝试提取列表项目
                        items = labels[idx].strip('[]').split(',')
                        labels_ = [item.strip().strip("'\"") for item in items]
                    else:
                        # 作为单个答案处理
                        labels_ = [labels[idx]]
            else:
