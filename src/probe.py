...
from probing_utils import extract_internal_reps_specific_layer_and_token, compile_probing_indices, \
    load_model_and_validate_gpu, get_probing_layer_names, LIST_OF_DATASETS,LIST_OF_TEST_DATASETS, LIST_OF_MODELS, \
...
def parse_args_and_init_wandb():
    ...
    parser.add_argument("--test_dataset", choices=LIST_OF_DATASETS+LIST_OF_TEST_DATASETS, required=False, default=None)
...
def main():
    ...
    if load_test:
        metrics_test = res[1]
        wandb.log({"step": 0})  # 添加步骤标记
        for m in metrics_test:
            wandb.log({f"{m}_test": metrics_test[m]})
