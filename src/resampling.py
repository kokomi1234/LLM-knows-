...
def main(args):
    ...
    output_prefix = f"../output/resampling/{MODEL_FRIENDLY_NAMES[args.model]}_{args.dataset}"
    
    # 保存不带n_resamples的文件，这样合并脚本可以找到它们
    if args.tag != "":
        # 如果有tag，使用it作为文件名一部分
        torch.save(all_textual_answers, f"{output_prefix}_2_textual_answers{tag_suffix}.pt")
        torch.save(all_input_output_ids, f"{output_prefix}_2_input_output_ids{tag_suffix}.pt")
        
        # 为了向后兼容，也保存带n_resamples参数的版本
        torch.save(all_textual_answers, f"{output_prefix}_{args.n_resamples}_textual_answers{tag_suffix}.pt")
        torch.save(all_input_output_ids, f"{output_prefix}_{args.n_resamples}_input_output_ids{tag_suffix}.pt")
    else:
        # 无tag的情况，直接使用标准命名
        torch.save(all_textual_answers, f"{output_prefix}_2_textual_answers.pt")
        torch.save(all_input_output_ids, f"{output_prefix}_2_input_output_ids.pt")
    ...
    if len(all_exact_answers['exact_answer']) != 0:
        ...
        if args.tag != "":
            torch.save(all_exact_answers, f"{output_prefix}_2_exact_answers{tag_suffix}.pt")
            torch.save(all_exact_answers, f"{output_prefix}_{args.n_resamples}_exact_answers{tag_suffix}.pt")
        else:
            torch.save(all_exact_answers, f"{output_prefix}_2_exact_answers.pt")
            
    print(f"Files saved to {output_prefix} with appropriate suffixes")
